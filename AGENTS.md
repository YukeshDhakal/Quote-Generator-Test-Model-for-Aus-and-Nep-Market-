...
- `packages/api` — Express + `node:sqlite` (NOT better-sqlite3 — see
  `.Codex/state/discoveries.md`). Auth (bcryptjs + JWT cookie + Google ID
...
- New jurisdiction = new profile file + two-line registration in the
  ...
  See `.Codex/skills/add-jurisdiction/`.
...
## How this project's `.Codex/` is organized
...
- Tasks live in Codex's native task tool, not markdown files.
